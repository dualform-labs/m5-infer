...
- **PyPI publish** — `pip install m5-infer` once name availability is confirmed.
- _Better cold-start TTFT_ — the first request on a fresh server is ~20 s due to RDMS draft load + Metal JIT. Explore overlapping draft load with initial prefill.
