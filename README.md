...
**Result**:
- ✅ 12 K-token tool schema **cold → warm TTFT: 69 s → 11 s** (6× from CTRSP cache hit on the second call). mlx_lm.server's in-process warm TTFT is 2.3 s (faster absolute); CTRSP's unique value is that the state **survives process restarts** — other engines lose their prefix cache at shutdown.
...
| **Warm TTFT** (12K tool schemas, 2nd call) | 64.9 s | **2.3 s** | 11.1 s |
...
**Raw data**: `docs/fullsuite_summary.json`
...
**Raw data**: `docs/thinking_summary.json`
...
**Raw data**: `docs/opus_quality_summary.json`
...
**結果**:
- ✅ 12 K token tool schema の **cold → warm TTFT: 69 秒 → 11 秒** (2 回目呼び出しの CTRSP ヒットで 6×)。mlx_lm.server の in-process warm TTFT は 2.3 秒で絶対値は速い。CTRSP 独自の価値は **プロセス再起動を越えて state が残る**ことで、他エンジンは shutdown 時に prefix cache を失います。
...
**生データ**: `docs/fullsuite_summary.json`
...
**生データ**: `docs/thinking_summary.json`
...
**生データ**: `docs/opus_quality_summary.json`
